...
def capture_handshake(interface, bssid, channel):
    """Captura un handshake de una red Wi-Fi."""
    command = [
        "airodump-ng", "-c", str(channel), "--bssid", bssid, "-w", "auditoria", interface
    ]
    subprocess.run(command, check=True)

def crack_password(cap_file, dictionary_file):
    """Realiza un ataque de diccionario sobre un archivo .cap."""
    command = ["aircrack-ng", "-w", dictionary_file, "-b", cap_file]
    result = subprocess.run(command, capture_output=True, text=True, check=True)
    # Extraer contraseña del output si es posible
    ...
    if "KEY FOUND" in output:
        ...
        raise Exception("Contraseña no encontrada")
